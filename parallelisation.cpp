...
void generate_data_range(int start, int end) {
...
void compute_sum_range(int start, int end, long long& sum) {
    sum = 0;
    for (int i = start; i < end; ++i) {
        sum += data[i];
    }
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        int start = i * range;
        int end = start + range;
        threads[i] = std::thread(generate_data_range, start, end);
    }
    for (int i = 0; i < NUM_THREADS; ++i) {
        threads[i].join();
    }
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        int start = i * range;
        int end = start + range;
        threads[i] = std::thread(compute_sum_range, start, end, std::ref(sums[i]));
    }
    for (int i = 0; i < NUM_THREADS; ++i) {
        threads[i].join();
    }
